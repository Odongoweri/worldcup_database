...
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON games.winner_id = teams.team_id WHERE round = 'Final' AND year = 2018 GROUP BY name")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE round = 'Eighth-Final' AND year = 2014 GROUP BY name")"
